...
**The film is 45.6MB, so it must be pushed with git.** GitHub's browser
upload refuses anything over 25MB, so dragging files into the web page
will not work. Git itself allows up to 100MB per file, so pushing is
fine.

Once you are signed in to GitHub on this computer:
...
The whole folder is about 23MB, which keeps it under GitHub's 25MB
per-file upload limit. The showreel is the bulk of that at 20MB. It is
a 35 second cut of the full film; the complete 78 second version lives
in the other copies of this project. If you ever need the folder
smaller still, put the film on YouTube and link to it instead, which
would bring this down to under 3MB.
